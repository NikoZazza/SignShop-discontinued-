...
class PlayerSignCreateEvent implements Listener{
    ...
    public function signChangeEvent(SignChangeEvent $event){
        ...
        if($line0 == "[signshop]" || $line0 == "/signshop"){
            ...
            if($this->SignMain->getProvider()->getPlayer($player->getDisplayName())["authorized"] == true){
                ...
                if($error == ""){
                    if($this->hasItemPlayer($player, Item::get($id, $damage, $line3)) == true){      
                        if($this->removeItemPlayer($player, Item::get($id, $damage, $line3)) == true){
    ...
    private function removeItemPlayer(Player $player, Item $item){
        if($this->SignMain->getProvider()->getPlayer($player->getDisplayName())["authorized"] == "super") return true;
